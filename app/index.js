  ...
  return function onProxyRes(proxyRes, req, res) {
    ...
    if (env.verbose) {
      ...
      res.end = function (chunk) {
        ...
        console.log(
          chalk.bold(chalk.cyan(who), req.path),
          chalk.gray(`\n[Output]`),
          env.formatJson ? JSON.stringify(JSON.parse(body), null, 2) : body
        );
...
function onProxyReq(proxyReq, req, res) {
  ...
  if (env.verbose) {
    if (env.showQueryString && _.keys(req.query).length) {
      ...
      console.log('\n');
      console.log(
        chalk.gray('[QueryString]'),
        JSON.stringify(req.query, null, env.formatJson ? 2 : null)
      );
    }
    ...
    if (env.showBody && req.method === 'POST' && _.keys(req.body).length) {
      ...
      console.log(
        chalk.gray('[PostBody]'),
        JSON.stringify(req.body, null, env.formatJson ? 2 : null)
      );
    }
  }
